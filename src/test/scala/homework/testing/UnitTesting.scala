...
import homework.testing.hal9000.HAL9000
...
//
// `Specs2` - one of the oldest libraries with opinionated Cucumber-like DSL,
...
class Exercise3Spec extends AnyWordSpec {
  "A calculator" when {
    ...
    "I am not sure about Scala" should {
      "quit?" in {
        val calculator = Calculator()
        assert(calculator.enter(5) == Right(Calculator(5, 0, None)))
      }
    }
    "does nothing" should {
      "when you just repeat pressing `=`" in {
...
class Exercise4Spec extends AnyFreeSpec with Matchers {
  ...
  "calculator" - {
    "enters the number correctly" in {
      val calculator = Calculator()
      calculator.enter(2) shouldBe Right(Calculator(1, 0, None))
...
// > syntax's that bloat the library and make it harder to developers to pick up,
...
class Exercise6Spec extends AnyFunSuite {
  ...
  test("Entering numbers in Calculator should work correctly") {
    val calculator = Calculator()
    assert(calculator.enter(1) == Right(Calculator(1, 0, None)))
    assert(calculator.enter(7) == Right(Calculator(7, 0, None)))
    assert(calculator.enter(12) == Left("digit out of range"))
  ...
  test("Calculator does nothing when '=' is pressed repeatedly") {
    val calculator = Calculator()
    assert(calculator.calculate.calculate.calculate.calculate == calculator)
  }
...
class Exercise8Spec extends AnyFunSuite {
  ...
  test("HAL9000 behaves as expected when asked to open the door") {
    assertThrows[RuntimeException](HAL9000.letAstronautIn())
  }
...
//
// HAL9000 goes crazy about his mission if two registers do not match.
...
class Exercise9Spec extends AnyFunSuite {
  ...
  test("HAL9000 working correctly") {
    assert(HAL9000.register1 == HAL9000.register2, "Robot has malfunctioned, run!")
  }
  ...
  object PlayerService {
    ...
    def apply(repository: PlayerRepository, logging: Logging): PlayerService = new PlayerService {
      ...
      def deleteWorst(minimumScore: Int) =
        repository.all.foreach(player => if (player.score < minimumScore) repository.delete(player.id))
      def celebrate(bonus: Int) =
        repository.all.foreach(player => repository.update(player.copy(score = player.score + bonus)))
    }
  }
}
...
class Exercise10Spec extends AnyFunSuite {
  ...
  class Fixture {
    val repository = new PlayerRepository {
      ...
      override def update(player: Player): Unit = players = players.map(old =>
        if (old.id == player.id) player
        else old
      )
      override def delete(id: String): Unit = players = players.filterNot(_.id == id)
    }
  ...
  test("PlayerService.deleteWorst works correctly") {
    ...
    // perform the test
    service.deleteWorst(30)

    // validate the results
    assert(f.repository.all == List(
      Player("2", "Ann", "a@a", 30),
      Player("3", "Peter", "a@a", 40)
    ))
  }
...
object Exercise12 {
  ...
  def dogs: Future[Int] = ???
  def message1: Future[String] = dogs.map(dogs => s"We have $dogs of dogs")
  // 2. Two `Future` classes could be combined by `flatMap`:
  def cats: Future[Int] = ???
  def message2: Future[String] = dogs.flatMap(dogs => cats.map(cats => s"We have $dogs of dogs and $cats of cats"))
  ...
  object PlayerService {
    ...
    def apply(repository: PlayerRepository, logging: Logging): PlayerService = new PlayerService {
      ...
      def deleteWorst(minimumScore: Int) =
        repository.all.map(_.filter(_.score < minimumScore).map(badPlayer => repository.delete(badPlayer.id)))

      def celebrate(bonus: Int) =
        repository.all.map(_.map(player => repository.update(player.copy(score = player.score + bonus))))
    }
  }
...
class Exercise12Spec extends AsyncFunSuite {
  ...
  class Fixture {
    ...
    val repository = new PlayerRepository {
      var players: AtomicReference[List[Player]] = new AtomicReference(List(
        Player("1", "John", "a@a", 10),
        Player("2", "Ann", "a@a", 30),
        Player("3", "Peter", "a@a", 40),
        Player("4", "Dick", "a@a", 20)
      ...
      def all = {
        val promise: Promise[List[Player]] = Promise()
        def future2: Future[List[Player]] = promise.future

        // How clumsy of me not to complete the promise ...
        future2
      }
      def update(player: Player) = Future {
        players.set(players.get().map(old =>
          if (old.id == player.id) player
          else old
        ))
      }
    ...
    val logging = new Logging {
      def info(message: String) = Future { println(message) }
    }
  }

  test("PlayerService.deleteWorst will work eventually") {

    // construct fixture
    val f = new Fixture
    val service = PlayerService(f.repository, f.logging)

    // perform the test and validate
    for {
      _      <- service.deleteWorst(30)
      result <- f.repository.all
    } yield assert(result == List(
      Player("2", "Ann", "a@a", 30),
      Player("3", "Peter", "a@a", 40)
    ))
  }
  test("PlayerService.celebrate will work eventually") {
    ...
    def apply[F[_]: Monad](repository: PlayerRepository[F], logging: Logging[F]): PlayerService[F] =
      new PlayerService[F] {
        ...
        def deleteWorst(minimumScore: Int) =
          repository.all.map(_.filter(_.score < minimumScore).map(badPlayer => repository.delete(badPlayer.id)))
        def celebrate(bonus: Int) =
          repository.all.map(_.foreach(player => repository.update(player.copy(score = player.score + bonus))))
...
// `F` calls by `Future` to prove it actually works.
//
...
class Exercise14FutureSpec extends AsyncFunSuite {
  ...
  class Fixture {
    val repository = new PlayerRepository[Future] {
      ...
      def update(player: Player) = Future(players.set(players.get().map(old =>
        if (old.id == player.id) player
        else old
      )))
  ...
  test("PlayerService.deleteWorst will work eventually") {
    ...
    // construct fixture
    val f = new Fixture
    val service = PlayerService[Future](f.repository, f.logging)
...
class git extends AnyFunSuite {
  ...
  class Fixture {
    ...
    val repository = new PlayerRepository[Option] {
      ...
      def byId(id: String) = Option(players.get().find(_.id == id))
      def all = Option({
        Thread.sleep(10000)
        players.get()
      })
      def update(player: Player) = Option(players.set(players.get().map(old =>
        if (old.id == player.id) player
        else old
      )))
      def delete(id: String) = Option(players.set(players.get().filterNot(_.id == id)))
    }
    val logging = new Logging[Option] {
      def info(message: String) = Option(println(message))
    }
  }
  ...
  test("PlayerService.deleteWorst will work eventually") {
    ...
    // construct fixture
    val f = new Fixture
    val service = PlayerService[Option](f.repository, f.logging)
...
class Exercise14StateSpec extends AnyFunSuite {
  ...
  class Fixture {
    ...
    val logging = new Logging[F] {
      def info(message: String) = State.pure(())
    }
  }
